...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    line_img = np.zeros((*img.shape, 3), dtype=np.uint8)  # 3-channel RGB image  创建一个全黑的画板
...
def annotate_image_array(image_in):
    ...
    cv2.imshow("sadf ", blur_gray)
    cv2.waitKey(5000)
    # Apply Canny Edge Detector
    edges = canny(blur_gray, low_threshold, high_threshold)  # 所有区域的 canny 边缘
    ...
    vertices = np.array([[
        ...
        , dtype=np.int32)
    masked_edges = region_of_interest(edges, vertices)  # ROI区域的 canny, 基于规则和先验进行提取
...
def annotate_image(input_file):
    ...
    # annotate_image_array(mpimg.imread(input_file).astype(np.uint8))
    annotate_image_array(cv2.imread(input_file).astype(np.uint8))
